...
- **AI Gateway** — Multi-provider LLM support, streaming responses, agent loop with sub-agent delegation, session modes, parallel tool execution, durable prompt queue that replays messages sent during a run as a single turn
...
- **Extensibility** — MCP servers (stdio + HTTP/SSE), Context7 documentation tools, built-in DuckDuckGo web search, skill system, 14 lifecycle hook events with circuit breaker
...
authentication, sandbox lifecycle, and container networking. See
[Generative UI with A2UI](docs/src/a2ui.md) for interactive chat surfaces,
action routing, validation, persistence, and troubleshooting.
