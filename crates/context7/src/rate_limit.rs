//! Shared Context7 rate-limit coordination for the client.
...
/// Upper bound for one shared Context7 rate-limit cooldown.
pub(crate) const CONTEXT7_MAX_RATE_LIMIT_COOLDOWN_MS: u64 = 60_000;
...
/// Shared gate that coordinates one Context7 rate limit across concurrent calls.
...
impl RateLimitCoordinator {
        ...
        loop {
            ...
            match action {
                AcquireAction::Ready { probe } => {
                    #[cfg(feature = "tracing")]
                    if probe {
                        tracing::debug!("Context7 shared rate-limit probe admitted");
                    }
                ...
                AcquireAction::SleepUntil(until) => {
                    ...
                    tracing::debug!(
                        ...
                        "Context7 request waiting for shared rate-limit cooldown"
                    );
                    tokio::select! {
                        () = sleep_until(until) => {},
                ...
                AcquireAction::WaitForChange => {
                    #[cfg(feature = "tracing")]
                    tracing::debug!("Context7 request waiting for shared rate-limit probe");
    ...
    fn complete(&self, probe: bool, rate_limited: bool, cooldown_ms: Option<u64>) {
        if rate_limited {
            if let Some(cooldown_ms) = cooldown_ms {
                if !self.start_cooldown(cooldown_ms) && probe {
                    self.release_probe();
                }
            } else if probe {
                self.release_probe();
    ...
    fn start_cooldown(&self, cooldown_ms: u64) -> bool {
        let cooldown_ms = cooldown_ms.min(CONTEXT7_MAX_RATE_LIMIT_COOLDOWN_MS);
        let Some(until) = Instant::now().checked_add(Duration::from_millis(cooldown_ms)) else {
            #[cfg(feature = "tracing")]
            tracing::warn!(
                cooldown_ms,
                "Context7 shared rate-limit cooldown deadline is not representable"
            );
            return false;
        };
        let changed = {
        ...
        if changed {
            ...
            tracing::warn!(
                cooldown_ms,
                "Context7 shared rate-limit cooldown started or extended"
            );
            self.signal_change();
        }
        true
    ...
    fn release_probe(&self) {
        ...
        if changed {
            #[cfg(feature = "tracing")]
            tracing::debug!("Context7 shared rate-limit probe released");
            self.signal_change();
        }
    }
...
impl Drop for RateLimitPermit<'_> {
    fn drop(&mut self) {
        if self.probe && !self.completed {
            #[cfg(feature = "tracing")]
            tracing::warn!(
                "Context7 shared rate-limit probe ended without a response; reopening the gate"
            );
