...
pub(super) fn check_semantic_warnings(config: &ChelixConfig, diagnostics: &mut Vec<Diagnostic>) {
    ...
    if config.tools.context7.request_timeout_secs == 0 {
        diagnostics.push(Diagnostic {
            severity: Severity::Error,
            category: "invalid-value",
            path: "tools.context7.request_timeout_secs".into(),
            message: "tools.context7.request_timeout_secs must be at least 1".into(),
        });
    }
    ...
    if config.tools.linkup.request_timeout_secs == 0 {
        diagnostics.push(Diagnostic {
            severity: Severity::Error,
            category: "invalid-value",
            path: "tools.linkup.request_timeout_secs".into(),
            message: "tools.linkup.request_timeout_secs must be at least 1".into(),
        });
    }
