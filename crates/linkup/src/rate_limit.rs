//! Shared Linkup `429 Too Many Requests` cooldown coordination.
...
/// Upper bound for one shared Linkup rate-limit cooldown.
pub(crate) const LINKUP_MAX_RATE_LIMIT_COOLDOWN_MS: u64 = 60_000;
...
/// Shared gate that coordinates one Linkup rate limit across concurrent calls.
...
impl RateLimitCoordinator {
        ...
        loop {
            ...
            match action {
                AcquireAction::Ready { probe } => {
                    #[cfg(feature = "tracing")]
                    if probe {
                        tracing::debug!("Linkup shared rate-limit probe admitted");
                    }
                ...
                AcquireAction::SleepUntil(until) => {
                    ...
                    tracing::debug!(
                        ...
                        "Linkup request waiting for shared rate-limit cooldown"
                    );
                    tokio::select! {
                        () = sleep_until(until) => {},
                ...
                AcquireAction::WaitForChange => {
                    #[cfg(feature = "tracing")]
                    tracing::debug!("Linkup request waiting for shared rate-limit probe");
    ...
    fn start_cooldown(&self, cooldown_ms: u64) -> bool {
        let cooldown_ms = cooldown_ms.min(LINKUP_MAX_RATE_LIMIT_COOLDOWN_MS);
        let Some(until) = Instant::now().checked_add(Duration::from_millis(cooldown_ms)) else {
            #[cfg(feature = "tracing")]
            tracing::warn!(
                cooldown_ms,
                "Linkup shared rate-limit cooldown deadline is not representable"
            );
            return false;
        };
        let changed = {
        ...
        if changed {
            ...
            tracing::warn!(
                cooldown_ms,
                "Linkup shared rate-limit cooldown started or extended"
            );
            self.signal_change();
        }
        true
    ...
    fn release_probe(&self) {
        ...
        if changed {
            #[cfg(feature = "tracing")]
            tracing::debug!("Linkup shared rate-limit probe released");
            self.signal_change();
        }
    }
...
impl Drop for RateLimitPermit<'_> {
    fn drop(&mut self) {
        if self.probe && !self.completed {
            #[cfg(feature = "tracing")]
            tracing::warn!(
                "Linkup shared rate-limit probe ended without a response; reopening the gate"
            );
