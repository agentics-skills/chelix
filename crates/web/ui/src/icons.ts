// ── Shared icons (CSS mask-image) ────────────────────────
export function makeTelegramIcon(): HTMLSpanElement {
	const el = document.createElement("span");
	el.className = "icon icon-telegram";
	return el;
}

export function makeTeamsIcon(): HTMLSpanElement {
	const el = document.createElement("span");
	el.className = "icon icon-msteams";
	return el;
}

export function makeDiscordIcon(): HTMLSpanElement {
	const el = document.createElement("span");
	el.className = "icon icon-discord";
	return el;
}

export function makeSlackIcon(): HTMLSpanElement {
	const el = document.createElement("span");
	el.className = "icon icon-slack";
	return el;
}

export function makeMatrixIcon(): HTMLSpanElement {
	const el = document.createElement("span");
	el.className = "icon icon-matrix";
	return el;
}

export function makeNostrIcon(): HTMLSpanElement {
	const el = document.createElement("span");
	el.className = "icon icon-nostr";
	return el;
}

export function makeCronIcon(): HTMLSpanElement {
	const el = document.createElement("span");
	el.className = "icon icon-cron";
	return el;
}

export function makeBranchIcon(): HTMLSpanElement {
	const el = document.createElement("span");
	el.className = "icon icon-branch";
	return el;
}

export function makeProjectIcon(): HTMLSpanElement {
	const el = document.createElement("span");
	el.className = "icon icon-xs icon-folder";
	return el;
}

export function makeChatIcon(): HTMLSpanElement {
	const el = document.createElement("span");
	el.className = "icon icon-chat";
	return el;
}

export function makeArchiveIcon(): HTMLSpanElement {
	const el = document.createElement("span");
	el.className = "icon icon-archive";
	return el;
}
