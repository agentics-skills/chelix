...
pub struct TelegramAccountConfig {
    ...
    /// Default model ID for this bot's sessions (e.g. "gemini-2.5-pro").
    /// When set, channel messages use this model instead of the first registered provider.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub model: Option<String>,

    /// Provider name associated with `model` (e.g. "gemini").
...
mod tests {
    ...
    fn overrides_round_trip() {
        let json = serde_json::json!({
            ...
            "user_overrides": {
                "456": { "model": "gemini-2.5-pro", "model_provider": "gemini", "agent_id": "user-agent" }
            }
        });
        ...
        assert_eq!(cfg.user_model("456"), Some("gemini-2.5-pro"));
        assert_eq!(cfg.user_model_provider("456"), Some("gemini"));
        ...
        assert_eq!(cfg2.user_model("456"), Some("gemini-2.5-pro"));
