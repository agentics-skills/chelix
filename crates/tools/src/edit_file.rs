//! `edit_file` agent tool backed exclusively by the managed tools service.
    ...
    fn parameters_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "edit": {
                    "description": "The exact replacement operation. Use the first form for a unique match or the second form to pass replaceAll explicitly.",
                    "oneOf": [
                        {
                            "type": "object",
                            "additionalProperties": false,
...
fn parse_input(mut params: Value) -> anyhow::Result<EditFileRequest> {
    let map = params
        .as_object_mut()
        .ok_or_else(|| anyhow::anyhow!("edit_file parameters must be an object"))?;
    map.retain(|key, _| !key.starts_with('_'));
    let request: EditFileRequest = serde_json::from_value(params)
        .map_err(|error| anyhow::anyhow!("invalid edit_file parameters: {error}"))?;
    request
        .validate()
        .map_err(|error| anyhow::anyhow!("invalid edit_file parameters: {error}"))?;
    Ok(request)
...
#[cfg(feature = "metrics")]
fn record_metrics(result: &crate::Result<EditFileResponse>) {
    match result {
        Ok(_) => {
            counter!(
                tools_metrics::EXECUTIONS_TOTAL,
                labels::TOOL => "edit_file".to_string(),
...
fn record_metrics(_result: &crate::Result<EditFileResponse>) {}
...
mod tests {
    ...
    fn exposes_name_and_strict_schema() {
        let tool = EditFileTool::new(client("http://127.0.0.1:1".into(), "unused"));

        assert_eq!(tool.name(), "edit_file");
        let schema = tool.parameters_schema();
        assert_eq!(schema["required"], json!(["filePath", "edit"]));
        assert_eq!(schema["additionalProperties"], false);
        assert_eq!(schema["properties"]["filePath"]["type"], "string");
        let variants = schema["properties"]["edit"]["oneOf"]
            .as_array()
            .unwrap_or_else(|| panic!("edit.oneOf must be an array"));
    ...
    fn parse_input_rejects_non_object_missing_null_unknown_and_invalid_fields() {
        for invalid in [
            ...
            json!({
                "filePath": "/tmp/file",
                "edit": { "oldString": "", "newString": "new" }
            }),
            json!({
                "filePath": "/tmp/file",
                "edit": { "oldString": "same", "newString": "same" }
            }),
            json!({
                "filePath": "/tmp/file",
                "oldString": "old",
    ...
    #[test]
    fn parse_input_accepts_both_nested_forms_and_strips_only_internal_context() {
        ...
        assert_eq!(request.edit.old_string(), "old");
        assert_eq!(request.edit.new_string(), "new");
        assert!(!request.edit.replace_all());
        ...
        .unwrap_or_else(|error| panic!("explicit parse failed: {error}"));
        assert!(request.edit.replace_all());
    }
    ...
    async fn execute_routes_session_and_returns_structured_result() {
        ...
        let call = server
            .mock("POST", chelix_protocol::TOOLS_SERVICE_EDIT_FILE_PATH)
            .match_header("authorization", "Bearer edit-token")
            ...
            .with_body(
                "{\"filePath\":\"/workspace/file.txt\",\"replacements\":1,\"replaceAll\":false,\"recovery\":\"crlf\"}",
            )
            .expect(1)
            .create_async()
            .await;
        let tool = EditFileTool::new(client(server.url(), "edit-token"));
        ...
        assert_eq!(
            ...
            json!({
                ...
                "replacements": 1,
                "replaceAll": false,
                "recovery": EditFileRecovery::Crlf
            })
        );
    ...
    async fn execute_surfaces_service_failure() {
        ...
        let call = server
            ...
            .with_status(422)
            .with_header("content-type", "application/json")
            .with_body("{\"error\":\"oldString matches 2 locations\"}")
            .expect(1)
            .create_async()
            .await;
