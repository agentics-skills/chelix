...
use {
    ...
    chelix_oauth::{OAuthTokens, TokenStore, kimi_headers},
    futures::StreamExt,
...
impl KimiCodeProvider {
    ...
    async fn get_valid_oauth_token(&self) -> anyhow::Result<String> {
        ...
        let tokens = token_store.load(PROVIDER_NAME).ok_or_else(|| {
            anyhow::anyhow!(
                "not logged in to kimi-code — run `chelix auth login --provider kimi-code`"
            )
        })?;
...
impl LlmProvider for KimiCodeProvider {
    ...
    fn stream_with_tools(
        ...
        Box::pin(async_stream::stream! {
            ...
            if !tools.is_empty() {
                match to_openai_tools(&tools) {
                    Ok(prepared) => body["tools"] = serde_json::Value::Array(prepared),
                    Err(error) => {
