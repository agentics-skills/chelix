...
#[tokio::test]
#[ignore]
async fn google_ai_studio_accepts_a_nested_union_tool_schema() {
    let key = api_key();
    let client = reqwest::Client::new();
    let tools = to_openai_tools(&[serde_json::json!({
        "name": "cron",
    ...
    assert!(
        status.is_success(),
        "OpenRouter Google AI Studio should accept a nested union tool schema, got {status}: {text}"
    );
}
