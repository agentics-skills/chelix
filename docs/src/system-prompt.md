...
Resolved by `chelix_projects::context::load_context_files()`. The loader walks
...
To load a skill, call the `read_skill` tool with its name
...
Optional markdown files from the data directory (`~/.chelix/`):
