...
A terminal running a command returns an explicit busy error. A managed run stays
busy across every line of its submitted payload, including continuation prompts.
An unfinished shell construct can be continued with `process.paste` or interrupted
with `process.send_keys`. Terminal IDs are
...
For a single OSC 633 command-start boundary, `execute_command` returns only the
screen output between command start (`C`) and command finish (`D`). This also
applies to a heredoc, a multiline shell construct, and multiple shell operations
executed within that boundary. Before the first `C`, the command output is empty;
while the command executes, the range extends through the current screen line.
Once a run observes multiple command-start boundaries, its result expands to a
continuous screen range from submission through the final primary prompt,
including entered commands, intermediate prompts, and output. An active
multicommand run extends that range through the current screen line. The first
payload line is written before capturing a background response. That snapshot
is fixed before feeding subsequent lines as the shell requests input.
`read_terminal_output` renders retained screen text and optionally limits the
result to the most recent `maxLines` logical lines.
A saved managed-run failure is returned alongside the history in the response's
`error` field. Agent-facing formatting displays the lifecycle status, then the
error, then the output, redacting secret environment values from both the output
and the error. The terminal remains available for direct input, subscriptions,
and inventory; a subsequent `execute_command` reports the saved failure.

The shell emits OSC 633 primary-prompt (`A/B`), continuation-prompt (`F/G`), and
command-boundary (`C/D`) events. A primary prompt acknowledges input after a
command-finish event. A changed `ChelixPromptId` property acknowledges a new
continuation prompt; redisplay repeats the same identifier. Prompt identifiers
use Bash arithmetic expansion and require the `promptvars` shell option. If the
option is disabled during a session, the unexpanded integration expression after
a command-finish event produces an explicit saved shell-integration failure.
The shell option remains under direct interactive control.
The service feeds one payload line at a time, waiting for a new primary or
continuation prompt before sending the next. A run records the observed
command-boundary counts in completion diagnostics, retains the last exit status,
and completes at the primary prompt after the last payload line. An exhausted
payload at a continuation prompt remains active for interactive completion.
Feeding continues after a background response or a blocking wait timeout.
Terminal replies generated by the RMUX emulator are written back to the same PTY.
