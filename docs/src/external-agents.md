...
Chelix can bind a chat session to an external CLI coding agent. When a session is bound, `chat.send` persists the user turn in Chelix, sends the prompt and recent session context to the external process, streams the CLI output back to the web UI, and persists the assistant response.
...
- Live external processes are not restored automatically after a Chelix gateway restart.
