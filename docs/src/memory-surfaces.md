...
Chelix has several different places where information can persist. They solve
...
`session_state` is Chelix's short-term memory layer. It is:
...
stores the canonical user profile in `chelix.toml [user]`, then overlays
