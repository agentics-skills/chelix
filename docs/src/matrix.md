...
That means Chelix cannot reliably decrypt encrypted rooms when you connect with
an access token copied out of Element.

Use password auth so Chelix logs in as its own Matrix device and persists its
own E2EE keys locally. After that:

- If Element starts verification, Chelix accepts it automatically
- Chelix posts emoji confirmation instructions in the Matrix chat
...
For dedicated bot accounts, the web UI now defaults to **Let Chelix own this
Matrix account**. In that mode Chelix bootstraps cross-signing and recovery for
...
Chelix uses the matrix-sdk built-in OAuth 2.0 flow (MSC3861):
...
Before configuring Chelix, you need a Matrix bot account:
...
passwords, never commit them to version control. Chelix stores them with
...
does not transfer that device's local private E2EE keys into Chelix. For
encrypted rooms, use `user_id` + `password` so Chelix creates and persists its
own Matrix device.
```
...
For encrypted Matrix chats, Chelix must behave like its own Matrix device with
its own persistent crypto state.

Password auth works for that flow because Chelix logs in as a fresh Matrix
...
Chelix does not currently implement recovery-key entry or room-key import, and
neither feature would make access-token auth equivalent to a proper dedicated
Chelix device anyway. The supported path for encrypted Matrix chats is still:

1. Add the Matrix account with `user_id` + `password`
2. Let Chelix create its own Matrix device
3. Complete Element verification with that Chelix device
4. Send new encrypted messages after verification

## Ownership Modes

When you add a password-based Matrix account, Chelix offers two ownership modes:
...
- self-sign the Chelix device after takeover succeeds
...
browser approval before Chelix is allowed to reset and take over cross-signing.
...
In this mode Chelix still connects and can chat, but it does not try to take
...
`data_dir()/chelix.db`. Matrix secrets are still handled as secrets in code,
...
- Chelix auto-generates the internal `account_id`
...
channels. If voice transcription is not configured, Chelix replies with setup
...
When Chelix needs to ask the user to choose from a short list of actions, the
...
If a generated interactive message exceeds that limit, Chelix falls back to a
...
2. Chelix generates a 6-digit OTP challenge
...
- Look at logs: `RUST_LOG=chelix_matrix=debug chelix`
...
- Remove the Matrix account from Chelix
- Re-add it with `user_id` + `password`
- Verify the new Chelix device in Element
- Resend a brand new encrypted message after verification

### Element says it is waiting for Chelix to accept verification
